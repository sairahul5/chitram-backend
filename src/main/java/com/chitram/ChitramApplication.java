                        ...
                        try {
                                ...
                                jdbcTemplate.execute(
                                                """
                                                                ...
                                                                """);
                                jdbcTemplate.execute(
                                                "CREATE INDEX IF NOT EXISTS idx_pin_likes_user ON pin_likes (user_id)");
                                jdbcTemplate.execute(
                                                "CREATE INDEX IF NOT EXISTS idx_pin_likes_pin ON pin_likes (visual_item_id)");
                                jdbcTemplate.execute(
                                                "CREATE INDEX IF NOT EXISTS idx_pin_likes_created ON pin_likes (created_at)");
                                jdbcTemplate.execute(
                                                "CREATE INDEX IF NOT EXISTS idx_visual_items_feed ON visual_items (moderation_status, id DESC)");
                                jdbcTemplate.execute(
                                                "CREATE INDEX IF NOT EXISTS idx_visual_items_recommendations ON visual_items (moderation_status, created_at DESC, id DESC)");
                                jdbcTemplate.execute(
                                ...
                                jdbcTemplate.execute(
                                                "CREATE INDEX IF NOT EXISTS idx_user_interactions_pin_type ON user_interactions (visual_item_id, interaction_type)");
                                jdbcTemplate.execute(
                                                "CREATE INDEX IF NOT EXISTS idx_user_interests_user_category_lower ON user_interests (user_id, LOWER(category))");
                                jdbcTemplate.execute("""
                                                ...
                                                """);
                                jdbcTemplate.execute(
                                                "ALTER TABLE app_settings ADD COLUMN IF NOT EXISTS setting_value VARCHAR(120)");
                                jdbcTemplate.update(
                                ...
                                jdbcTemplate.execute(
                                                "ALTER TABLE visual_items ADD COLUMN IF NOT EXISTS share_key VARCHAR(36)");
                                jdbcTemplate.update(
                                                "UPDATE visual_items SET share_key = gen_random_uuid()::text WHERE share_key IS NULL");
                                jdbcTemplate.execute(
                                ...
                                jdbcTemplate.execute(
                                                "ALTER TABLE reports ADD COLUMN IF NOT EXISTS description VARCHAR(1000)");
                                jdbcTemplate.execute(
                                ...
                                jdbcTemplate.execute(
                                                "CREATE INDEX IF NOT EXISTS idx_reports_duplicate_check ON reports (reporter_id, target_type, target_id, status)");
                                jdbcTemplate.execute(
                                ...
                                String message = cause.getMessage() == null ? exception.getMessage()
                                                : cause.getMessage();
                                throw new IllegalStateException("Chitram database connection failed: " + message,
                                                exception);
                        }
                };
        }
