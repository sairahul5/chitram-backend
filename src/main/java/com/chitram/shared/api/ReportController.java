...
public class ReportController {
    ...
    private static final Set<String> REASONS = Set.of("SPAM", "NUDITY", "VIOLENCE", "HARASSMENT", "COPYRIGHT",
    ...
    public ReportController(JdbcTemplate jdbcTemplate, UserProfileService userProfileService,
    ...
    public ResponseEntity<Void> report(@AuthenticationPrincipal OAuth2User principal,
        ...
        if (jdbcTemplate.queryForObject("SELECT COUNT(*) FROM " + targetTable + " WHERE id = ?", Integer.class,
                request.targetId()) == 0) {
            throw new IllegalArgumentException("Report target was not found");
        }
        Integer duplicate = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM reports WHERE target_type = ? AND target_id = ? AND reporter_id = ? AND status IN ('PENDING', 'REVIEWING')",
                Integer.class, targetType, request.targetId(), reporter.getId());
        if (duplicate != null && duplicate > 0) {
            return ResponseEntity.accepted().build();
        }
        jdbcTemplate.update(
                "INSERT INTO reports (visual_item_id, reported_by, target_type, target_id, reporter_id, reason, description) VALUES (?, ?, ?, ?, ?, ?, ?)",
                "PIN".equals(targetType) ? request.targetId() : null, reporter.getId(), targetType, request.targetId(),
                reporter.getId(), reason, description);
